...
MarbleList::MarbleList(const unsigned int players, const unsigned int max_marble)
    : m_players{ players }, m_max_marble{ max_marble }
{ }
...
unsigned int MarbleList::play_game() {
  ...
  typedef std::pair<unsigned int, unsigned int> MarblePlayer;
  auto marble_player_comp{
      [](const MarblePlayer p1, const MarblePlayer p2) { return p1.second < p2.second; }
  };
  auto winning_player = std::max_element(m_player_scores.begin(), m_player_scores.end(),
      marble_player_comp);
